...
function App() {
  ...
  function initHorses(): HorseInfo[] {
    ...
    for (let r = 0; r < 11; r++) {
      for (let c = 0; c < 11; c++) {
        ...
        // P1 배치 (대각선)
        if ((r === 0 && c <= 2) || (r <= 2 && c === 0))
          list.push({ owner: "PLAYER_1", position: pos });
        if ((r === 10 && c >= 8) || (r >= 8 && c === 10))
          list.push({ owner: "PLAYER_1", position: pos });
        // P2 배치
        if ((r === 0 && c >= 8) || (r <= 2 && c === 10))
          list.push({ owner: "PLAYER_2", position: pos });
        if ((r === 10 && c <= 2) || (r >= 8 && c === 0))
          list.push({ owner: "PLAYER_2", position: pos });
      }
    }
    ...
    if (!selectedPos) {
      ...
      if (isValidMove(selectedPos, clickedPos)) {
        ...
        const newHorses = horses.map((h) =>
          h.position === selectedPos ? { ...h, position: clickedPos } : h,
        );
        setHorses(newHorses);
  ...
  // 유틸리티
  function toCoord(r: number, c: number) {
  ...
  return (
    <div className="game-container">
      <div className="ui-panel">
        ...
        <p>
          Score - P1: {scores.p1} | P2: {scores.p2}
        </p>
      </div>

      <div
        className={`ui-panel ${currentTurn === "PLAYER_1" ? "white-turn" : "black-turn"}`}
      >
        ...
        <div className="rule-box">
          ...
          <div className="rule-tooltip">
            ...
            <ul>
              <li>
                <strong>목표:</strong> 정중앙 오아시스(파란색)에 먼저 도착!
              </li>
              <li>
                <strong>슬라이드:</strong> 상하좌우 직선으로 벽이나 다른 말에
                부딪힐 때까지 멈출 수 없음.
              </li>
              <li>
                <strong>L자 이동:</strong> 체스 나이트처럼 이동 (단, 초원은 L자
                이동으로 도착할 수 없음).
              </li>
            </ul>
          </div>
        ...
        {Array.from({ length: 11 }).map((_, r) =>
          Array.from({ length: 11 }).map((_, c) => {
            ...
            return (
              <div
                key={pos}
                className={`tile ${getTileType(r, c).toLowerCase()} ${selectedPos === pos ? "selected" : ""}`}
                onClick={() => handleTileClick(r, c)}
              >
                {horse && (
                  <img
                    src={horse.owner === "PLAYER_1" ? whiteHorse : blackHorse}
                    className="horse-img"
                    alt="horse"
                  />
                )}
